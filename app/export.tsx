...
async function importRows(rows: Record<string, string>[], seasonId: string): Promise<{ imported: number; skipped: number }> {
  let imported = 0;
  let skipped = 0;
...
export default function ExportScreen() {
  ...
  const exportCostsCsv = async () => {
    ...
    try {
      const headers = ["Date", "Description", "Category", "Subcategory", "Section", "Amount (KES)", "Qty", "Unit", "Unit Price", "Product", "Supplier", "Workers", "Days", "Rate/Day", "Historical", "Notes"];
      const rows = costs.map((c) => [c.cost_date, c.description, c.cost_category, c.cost_subcategory, c.section_id === "section-a" ? "Section A" : c.section_id === "section-b" ? "Section B" : "Both", c.amount_kes, c.quantity, c.unit, c.unit_price_kes, c.product_name, c.supplier, c.num_workers, c.days_worked, c.rate_per_worker_per_day, c.is_historical ? "Yes" : "No", c.notes]);
  ...
  const exportActivitiesCsv = async () => {
    ...
    try {
      ...
      const rows = activityLogs.map((l) => [l.actual_date, l.activity_name, l.section_id === "section-a" ? "Section A" : l.section_id === "section-b" ? "Section B" : "Both", l.num_workers, l.labor_cost_kes, l.total_cost_kes - l.labor_cost_kes, l.total_cost_kes, l.weather_conditions, l.is_deviation ? "Yes" : "No", l.notes]);
  ...
  const exportInventoryCsv = async () => {
    ...
    try {
      ...
      const rows = inventory.map((i) => { const remaining = i.quantity_purchased - (i.quantity_used || 0); return [i.product_name, i.category, i.quantity_purchased, i.unit, i.unit_price_kes, i.quantity_purchased * i.unit_price_kes, i.quantity_used, remaining, i.purchase_date, i.supplier, i.notes]; });
  ...
  const exportFullJson = async () => {
    ...
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
    try {
      const payload = { exported_at: new Date().toISOString(), farm: "Rift Valley Potato Farm", summary: { total_costs: costs.reduce((s, c) => s + c.amount_kes, 0), total_activities: activityLogs.length, total_harvest_bags: harvestRecords.reduce((s, r) => s + r.bags, 0), total_revenue: harvestRecords.reduce((s, r) => s + r.total_revenue_kes, 0) }, costs, activity_logs: activityLogs, inventory, observations, harvest_records: harvestRecords };
  ...
  const parseXlsxToRows = (base64Data: string): Record<string, string>[] => {
    ...
    return raw.map((r) => {
      const row: Record<string, string> = {};
      for (const k of Object.keys(r)) row[normalizeKey(k)] = String(r[k] ?? "").trim();
      return row;
    }).filter((row) => Object.values(row).some((v) => v !== ""));
  };
  ...
  const handlePickFile = async () => {
    setPickingFile(true);
    try {
      const result = await DocumentPicker.getDocumentAsync({ type: ["application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "application/vnd.ms-excel", "text/csv", "text/comma-separated-values"], copyToCacheDirectory: true, multiple: false });
      ...
      if (rows.length === 0) {
        Alert.alert("Nothing found", "The file appears empty or has no recognised columns.");
        return;
      }
      setPreviewRows(rows);
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      setPreviewRows(null);
      setPreviewFilename("");
      Alert.alert("Import complete", `${imported} record${imported === 1 ? "" : "s"} imported into ${activeSeason?.season_name || "the active season"}.${skipped > 0 ? ` ${skipped} row${skipped === 1 ? "" : "s"} skipped.` : ""}`);
    } catch {
  ...
  const handleImportCsvText = async () => {
    ...
    setImporting(true);
    try {
      const { imported, skipped } = await importRows(parseCsvText(text), seasonId);
      await refresh();
      setImportText("");
      Alert.alert("Imported", `${imported} record${imported === 1 ? "" : "s"} added.${skipped > 0 ? ` ${skipped} row${skipped === 1 ? "" : "s"} skipped.` : ""}`);
    } catch {
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        {previewRows ? (
          <View style={styles.previewCard}>
            ...
            <Text style={styles.previewCols} numberOfLines={3}>{Object.keys(previewRows[0] || {}).join(", ")}</Text>
            <View style={styles.previewRows}>
              {previewRows.slice(0, 3).map((row, i) => (
                <View key={i} style={styles.previewRow}>
                  {Object.entries(row).slice(0, 4).map(([k, v]) => (<Text key={k} style={styles.previewCell} numberOfLines={1}><Text style={styles.previewKey}>{k}:</Text> {v}</Text>))}
                </View>
              ))}
              {previewRows.length > 3 ? <Text style={styles.previewMore}>+{previewRows.length - 3} more rows…</Text> : null}
            </View>
            <View style={styles.previewActions}>
              <Pressable style={styles.cancelBtn} onPress={() => { setPreviewRows(null); setPreviewFilename(""); }}><Text style={styles.cancelBtnText}>Cancel</Text></Pressable>
          ...
          <View style={styles.importCard}>
            <Pressable style={({ pressed }) => [styles.pickBtn, (pickingFile || importing) && { opacity: 0.6 }, pressed && { opacity: 0.85 }]} onPress={handlePickFile} disabled={pickingFile || importing}>
            ...
            {showImportHelp ? (
              <View style={styles.helpCard}>
                ...
                {[["Costs", "type=cost · date · description · amount · category · subcategory · section (a/b) · qty · unit · unit_price · product · supplier · workers · days · rate · notes"], ["Inventory", "type=inventory · product · category · qty · unit · unit_price · used · date · supplier · notes"], ["Harvest", "type=harvest · date · bags · kg_per_bag · price_per_bag · revenue · section (a/b) · buyer · notes"], ["Activities", "type=activity · date · activity · section (a/b) · workers · labor_cost · total_cost · notes"], ["Observations", "type=observation · date · description · severity · observation_type · section (a/b) · action_taken"]].map(([label, cols]) => (<View key={label} style={styles.helpRow}><Text style={styles.helpRowLabel}>{label}</Text><Text style={styles.helpRowCols}>{cols}</Text></View>))}
