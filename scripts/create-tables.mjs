...
async function run() {
  console.log(`Creating tables in Supabase project: ${projectRef}\n`);
  ...
  if (res.ok) {
    ...
    console.log("\nManagement API failed. Trying via PostgREST RPC...");
    // Try via PostgREST if management API not available
    ...
    for (const stmt of stmts) {
      ...
      console.log(`  ${r2.status}: ${t.slice(0, 80)}`);
    }
  }
}
